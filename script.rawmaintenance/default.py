...
def mainMenu():
    ...
    addDir('Mantenciones','url', 5,os.path.join(mediaPath, "maintenance.png"))
    
def maintMenu():
    analytics.sendPageView("RawMaintenenance","maintenance","maint")
    xbmc.executebuiltin("Container.SetViewMode(500)")
    addItem('Limpiar el cache del systema','url', 1,os.path.join(mediaPath, "cache.png"))
    addItem('Eliminar paquetes descargados', 'url', 3,os.path.join(mediaPath, "packages.png"))
...
def clearCache():
    global analytics
    analytics.sendEvent("Mantenciones", "Limpiar el cache del systema")
    ...
    if os.path.exists(cachePath)==True:    
        for root, dirs, files in os.walk(cachePath):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Limpiar el cache", str(file_count) + " Archivos encontrados", "Quiere eliminar los archivos encontrados?"):
    ...
    if os.path.exists(tempPath)==True:    
        for root, dirs, files in os.walk(tempPath):
            ...
            if file_count > 0:
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Borra el cache generado por el systema", str(file_count) + " Archivos encontrados", "Quiere eliminar los archivos encontrados?"):
                    for f in files:
                        try:
                            if (f == "xbmc.log" or f == "xbmc.old.log"): continue
                            os.unlink(os.path.join(root, f))
        ...
        if os.path.exists(clear_cache_path)==True:    
            for root, dirs, files in os.walk(clear_cache_path):
                ...
                if file_count > 0:
                    ...
                    dialog = xbmcgui.Dialog()
                    if dialog.yesno("Raw Manager",str(file_count) + "%s Archivos encontrados"%(entry.name), "Quiere eliminar los archivos encontrados?"):
                        for f in files:
                            os.unlink(os.path.join(root, f))
                        for d in dirs:
                            shutil.rmtree(os.path.join(root, d))
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Mantencion Completa", "Archivos eliminados")
    
    
def deleteThumbnails():
    global analytics
    analytics.sendEvent("Mantencion", "Eliminar miniaturas")
    
    if os.path.exists(thumbnailPath)==True:  
            dialog = xbmcgui.Dialog()
            if dialog.yesno("Eliminar miniaturas", "Esta opcion elimina miniaturas de las aplicaciones y las reconstruira", "Seguro que quieres hacer esto?"):
    ...
    dialog.ok("Reinicie el systema", "Porfavor reinicie el systema para reconstruir la biblioteca en miniatura")
        
def purgePackages():
    global analytics
    analytics.sendEvent("Mantencion", "Eliminar paquetes")
    ...
    for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
    if dialog.yesno("Eliminar paquetes descargados por el systema", "%d Paquetes encontrados."%file_count, "Eliminar los paquetes encontrados?"):  
        for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
            if file_count > 0:            
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
                dialog = xbmcgui.Dialog()
                dialog.ok("Mantencion", "Todos los paquetes eliminados")
            else:
                dialog = xbmcgui.Dialog()
                dialog.ok("Mantencion", "No existen paquetes para eliminar")
